...
function Slider(slider) {
  ...
  function startSlider() {
    ...
    prev = current.previousElementSibling || slides.lastElementChild; // nextSibling gives a node and node can be a text or an ELement while NextElementSlibing gives the next element.
  ...
  function move(direction) {
    ...
    if (direction === 'back') {
      ...
      var _ref = [// get the prev slide and if not there than get the last slide of the slider
      prev.previousElementSibling || slides.lastElementChild, prev, current];
      prev = _ref[0];
      current = _ref[1];
      next = _ref[2];
    } else {
      var _ref2 = [current, next, // get the next slide and if not present than the loop around and get the firstChild.
  ...
  prevButton.addEventListener('click', function () {
    return move('back');
  }); // we need to pass an argument than use run arrow function
